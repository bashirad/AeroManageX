...
INSERT INTO user (user_name, user_password, role_id) VALUES ('John', 'password123', 1);
...
JOIN 
    Flight F ON B.Confirmation_ID = F.Confirmation_ID;
drop view scheduleDetails;
CREATE VIEW scheduleDetailsView AS 
SELECT 
	P.passenger_ID AS ID,
	Passenger_FName AS FName,
    Passenger_LName AS LName,
    F.Confirmation_ID AS Confirmation,
    Booking_Date AS BookingDate,
    Booking_Platform AS Platform,
    Flight_ID AS FlightNo,
    Org_Departure_Time AS DepTime,
    Flight_Time AS Duration
FROM
	booking B
JOIN
	passenger P ON P.Passenger_ID = B.Passenger_ID
JOIN
	flight F ON B.Passenger_ID = F.Confirmation_ID;
...
SELECT 
    ...
    Flight_Attendant.Flight_Att_ID AS Flight_Attendant_ID,
    Flight_Attendant.Flight_Att_FName AS Flight_Attendant_FName,
    Flight_Attendant.Flight_Att_LName AS Flight_Attendant_LName
FROM 
    Flight
LEFT JOIN 
...
LEFT JOIN 
    Flight_Attendant ON Flight.Flight_Att_ID = Flight_Attendant.Flight_Att_ID;
...
select * from Remove_Flight_View;

select * from usercontacts;
use mydb;
select * from user;

SELECT COUNT(user_name) FROM USER WHERE user_name = 'bashirad';